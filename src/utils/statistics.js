...
export const getUserRank = (movies) => {
  const moviesWatchedCount = filterMovie[FilterType.HISTORY](movies).length;
...
export const getDuration = (movies, type) => {
  const totalDuration = movies.reduce((totalTime, movie) => totalTime + movie.movieInfo.runtime, 0);
